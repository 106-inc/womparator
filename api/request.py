...
def request(llm_api: api.APIRequest, text = "", clauses = [], role = -1):
    ...
    results = []
    if role == 0:
        res = llm_api.request(text, "Ты професиональный опытный инженер. Твоя задача найти в тексте ниже строгие\
                            ...
                            Ответ должен быть максимально лаконичным и содержать только техническую информацию. Ответ должен быть на русском языке.")
        results.append(res)
    elif role == 1:
        a = llm_api.request(text, "Ты профессиональный опытный инженер. Сделай краткий перессказ данного\
                            технического документа и выдели всю важную информацию. Ответ должен быть максимально лаконичным. Ответ должен быть на русском языке.")
        results.append(a)
    else:
        for clause in clauses:
            a = llm_api.request(text, "Ты профессиональный опытный инженер. Ты на вход будешь получать текст с техническим описанием. \
                Определи удовлетворяет ли текст пункту технического требования. Ответами могут быть: нет информации (о пункте ничего не сказано в тексте), несоответствует (имеется противоречивая информация), частично соответствует (нехватает информации), соответствует (информация в пункте полностью соответствует информации текста). Ответ должен быть на русском языке.\
